...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
	    ...
	    #python -m espnet2.bin.tokenize_text --input text --output here --token_type bpe --bpemodel data/en_token_list/bpe_unigram5000/bpe.model
            cat corpus | lmplz -S "20%" --discount_fallback -o ${ngram_num} - >${ngram_exp}/${ngram_num}gram.arpa
            build_binary -s ${ngram_exp}/${ngram_num}gram.arpa ${ngram_exp}/${ngram_num}gram.bin
